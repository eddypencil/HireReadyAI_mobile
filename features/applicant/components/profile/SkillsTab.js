...
// Hardcoded distinct colors — NOT using c.accent so they don't get overridden
const CERT_BG = '#fef3c7';    // yellow/amber background
const CERT_COLOR = '#d97706'; // amber icon
const AWARD_BG = '#fff1f2';   // pink/rose background
const AWARD_COLOR = '#e11d48'; // rose icon
...
export default function SkillsTab({ profile, viewOnly, onEdit, onDelete }) {
  ...
  return (
    <View style={styles.container}>
              ...
              {skills.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!viewOnly && (
                      <TouchableOpacity onPress={() => onDelete('skills', index)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
      ...
      {/* Certificates — yellow/amber icon */}
      <View style={styles.section}>
          ...
          : certificates.map((item, index) => (
            <View key={index} style={styles.certCard}>
              <View style={styles.certRow}>
                <View style={[styles.certIcon, { backgroundColor: CERT_BG }]}>
                  <Ionicons name="ribbon-outline" size={16} color={CERT_COLOR} />
                </View>
        ...
        {awards.length === 0
          ...
          : awards.map((item, index) => (
            <View key={index} style={styles.certRow}>
              <View style={[styles.certIcon, { backgroundColor: AWARD_BG }]}>
                <Ionicons name="trophy-outline" size={16} color={AWARD_COLOR} />
              </View>
              <View style={styles.certInfo}>
...
function createStyles(c) {
  return StyleSheet.create({
    ...
    sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderBottomWidth: 1, borderBottomColor: c.border },
    sectionHeaderLeft: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    sectionTitle: { fontSize: 15, color: c.foreground, fontWeight: '700' },
    addBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: c.primary, alignItems: 'center', justifyContent: 'center' },
    ...
    skillPill: { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 },
    skillPillText: { fontSize: 13, fontWeight: '600' },
    skillLevel: { fontSize: 11, fontWeight: '500' },
    langRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderTopWidth: 1, borderTopColor: c.border },
    ...
    certCard: { borderTopWidth: 1, borderTopColor: c.border },
    certRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 14 },
    certIcon: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    ...
    rowActions: { flexDirection: 'column', gap: 8, flexShrink: 0, alignItems: 'center', justifyContent: 'center' },
    certImageContainer: { borderTopWidth: 1, borderTopColor: c.border },
    certImageGallery: { padding: 12, gap: 8 },
    certMediaTile: { width: 120, height: 90, borderRadius: 10, overflow: 'hidden', backgroundColor: c.surface, borderWidth: 1, borderColor: c.border },
    certMediaImg: { width: '100%', height: '100%' },
  });
}