...
const INTERVIEW_STAGE_TYPES = [
  'assessment_test', 'coding_test', 'video_interview',
  'technical_interview', 'hr_interview', 'manager_interview',
...
export default function ApplicantFeedbackPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('applications')
      .select(`id, current_stage, is_rejected, applied_at,
      ...
      .then(({ data, error }) => {
        if (!error && data) {
          const filtered = data.filter(a =>
            a.current_stage === 'rejected' || a.is_rejected === true || a.current_stage === 'hired'
          );
  ...
  useEffect(() => {
    ...
    getCandidateProfile(selectedId).then(async ({ data }) => {
      ...
      if (data?.job_postings?.id && data.composite_score != null && data.composite_score !== 0) {
        getJobScorePercentile(data.job_postings.id, data.composite_score)
          .then(({ percentile: p }) => setPercentile(p));
      }
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.selectorBar}>
        <AppSelector applications={applications} selectedId={selectedId} onSelect={setSelectedId} />
      </View>
        ...
        <>
          ...
          {activeTab === 0 && (
            <ScrollView style={styles.tabScroll} contentContainerStyle={styles.tabContent}>
              <CandidateHeader app={app} percentile={percentile} percentileTag={percentileTag} />
              <KeyTakeaways cvFeedback={cvFeedback} stages={stagesWithQuestions} />
              {allStages.length > 0 && <ApplicationTimeline stages={allStages} />}
              <SkillsToDevelop cvFeedback={cvFeedback} stages={stagesWithQuestions} />
