...
serve(async (req) => {
  ...
  if (!price_id || !company_id) {
    return new Response(
      JSON.stringify({
        error: "price_id and company_id are required",
      }),
      {
        status: 400,
        headers: {
  ...
  try {
    ...
    // Used by webhook to identify company
    ...
    params.set(
      "payment_intent_data[metadata][company_id]",
      company_id
    );
    ...
    params.set(
      "success_url",
      "hirereadyai://premium/success"
    );

    params.set(
      "cancel_url",
      "hirereadyai://premium/cancel"
    );
