...
const LEVELS = [
  { key: 'basic',          label: 'Basic',          desc: 'Elementary understanding' },
  { key: 'conversational', label: 'Conversational', desc: 'Can hold a conversation' },
  { key: 'fluent',         label: 'Fluent',         desc: 'Comfortable in most situations' },
  { key: 'native',         label: 'Native',         desc: 'Mother tongue or equivalent' },
];
...
export default function EditLanguagesScreen() {
  ...
  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('Required', 'Language name is required.'); return; }
    setSaving(true);
    try {
