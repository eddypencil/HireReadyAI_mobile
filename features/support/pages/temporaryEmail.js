...
// ── Gmail SMTP credentials (no domain verification needed, free, can send
// to any recipient). Set these as Supabase Edge Function secrets:
...
serve(async (req) => {
  ...
  try {
    ...
    // Note: Gmail requires the "from" address to match the authenticated
    // account, so we keep GMAIL_USER as the sender but set replyTo to the
    // recruiter's email so candidate replies go to the recruiter.
    await client.send({
